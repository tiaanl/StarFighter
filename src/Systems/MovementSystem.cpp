...
void MovementSystem::update(ju::EntityManager& entities, F32 adjustment) {
    // Move all ships.
    for (auto& entity : entities.allEntitiesWithComponent<PositionComponent, MovementComponent, CombatComponent>()) {
    ...
    for (auto& ammoEntity : entities.allEntitiesWithComponent<PositionComponent, AmmoComponent>()) {
        ...
        F32 dirInRadians = ca::degreesToRadians(position->rot - 90.f);
...
void MovementSystem::handleMoveToPosition(ju::EntityManager& entities, ju::Entity& entity, F32 adjustment) {
    ...
    position->rot = 180.f - ca::radiansToDegrees(ca::angleBetweenInRadians(position->pos, movement->targetPos));
